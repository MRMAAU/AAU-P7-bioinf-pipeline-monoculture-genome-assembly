...
if [ -f "${ANTISMASH_DIR}/index.html" ]; then
    ...
    
    {
        ...
        if [ -f "${ANTISMASH_DIR}/${SAMPLE}.json" ]; then
            ...
            TOTAL_BGCS=$(grep -o '"product": "[^"]*"' "${ANTISMASH_DIR}/${SAMPLE}.json" | wc -l)
        ...
        # List interesting cluster types
        ...
        grep -o '"product": "[^"]*"' "${ANTISMASH_DIR}/${SAMPLE}.json" | \
            ...
            sort | uniq -c || echo "  None of the typical antibiotic types detected"
