...
echo "======================================================"
echo "      ONT Assembly Pipeline Submission"
echo "======================================================"
...
for SAMPLE in "${SAMPLES[@]}"; do
    ...
    ID_ASM=$(sbatch --parsable \
        --job-name="1_asm_${SAMPLE}" \
        --output="${LOG_DIR}/1_asm_${SAMPLE}_%j.out" \
        --error="${LOG_DIR}/1_asm_${SAMPLE}_%j.err" \
        --cpus-per-task=16 \
        --mem=64G \
        --time=01:20:00 \
        "${SCRIPT_DIR}/01_assembly.sh" "${SAMPLE}" "5m")

    if [ -z "${ID_ASM}" ]; then
        echo "  ERROR: Assembly submission failed. Stopping for ${SAMPLE}."
        continue
    fi
    echo "  > [1/3] Assembly submitted   (Job ID: $ID_ASM)"

    # --- JOB 2: EVALUATION ---
    ID_EVAL=$(sbatch --parsable \
        --dependency=afterok:${ID_ASM} \
        --job-name="2_eval_${SAMPLE}" \
        --output="${LOG_DIR}/2_eval_${SAMPLE}_%j.out" \
        --error="${LOG_DIR}/2_eval_${SAMPLE}_%j.err" \
        --cpus-per-task=8 \
        --mem=32G \
        --time=02:00:00 \
        "${SCRIPT_DIR}/02_evaluation.sh" "${SAMPLE}")

    echo "  > [2/3] Evaluation submitted (Job ID: $ID_EVAL)"

    # --- JOB 3: DOWNSTREAM ---
    ID_DOWN=$(sbatch --parsable \
        --dependency=afterok:${ID_EVAL} \
        --job-name="3_mine_${SAMPLE}" \
        --output="${LOG_DIR}/3_mine_${SAMPLE}_%j.out" \
        --error="${LOG_DIR}/3_mine_${SAMPLE}_%j.err" \
        --cpus-per-task=16 \
        --mem=32G \
        --time=02:00:00 \
        "${SCRIPT_DIR}/03_downstream.sh" "${SAMPLE}")

    echo "  > [3/3] Downstream submitted (Job ID: $ID_DOWN)"
    echo "------------------------------------------------------"
done
